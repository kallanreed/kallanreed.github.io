...
/* Emscripten has no termcap/curses — force stub implementations */
#undef HAVE_TGETENT
...
static int edit(int chn, int output_nl) /*{{{*/
{
  ...
  char *buf=f->inBuf;
#ifdef __EMSCRIPTEN__
  /* Asyncify: suspend WASM and wait for JS to provide a line of input */
  extern char* bas_wasm_read_line(void);
  char *line = bas_wasm_read_line();
  if (!line) { f->inCapacity=0; FS_errmsg=(const char*)0; return -1; }
  size_t len = strlen(line);
  if (len >= sizeof(f->inBuf)) len = sizeof(f->inBuf)-1;
  memcpy(f->inBuf, line, len);
  /* ensure newline terminated */
  if (len==0 || f->inBuf[len-1]!='\n') { f->inBuf[len++]='\n'; }
  f->inCapacity = (int)len;
  free(line);
  /* echo the input to output, minus trailing newline */
  for (buf=f->inBuf; buf<(f->inBuf+f->inCapacity-1); ++buf) FS_putChar(chn,*buf);
  if (output_nl) FS_putChar(chn,'\n');
  return 0;
#else
  char ch;
  int r;
...
int FS_close(int dev) /*{{{*/
{
  ...
  if (file[dev]->tty) { /* no-op in EMSCRIPTEN build */ }
#ifndef __EMSCRIPTEN__
...
int FS_inkeyChar(int dev, int ms) /*{{{*/
{
  ...
  return 0;
#elif defined(__EMSCRIPTEN__)
  /* INKEY$ not supported in browser build — return no key */
  FS_errmsg=(const char*)0;
  return -1;
#else
  timedread=rawMode;
